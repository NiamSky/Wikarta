...
import type { BreadcrumbItem, MaintenanceLog, Odc } from '@/types';
...
export default function OdcShow({ odc, maintenanceLogs }: Props) {
    useFlash();
    ...
    const parentPoint = odc.olt && odc.olt.latitude != null && odc.olt.longitude != null
        ? {
            latitude: odc.olt.latitude,
            longitude: odc.olt.longitude,
            label: `Parent OLT: ${odc.olt.code}`,
        }
        : null;

    const hasMapContext = (odc.latitude != null && odc.longitude != null) || parentPoint !== null;

    function submitMaintenance(e: React.FormEvent) {
        e.preventDefault();
        maintenanceForm.post(`/odcs/${odc.id}/maintenance-logs`, {
            preserveScroll: true,
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="p-6 space-y-6">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <Button variant="ghost" size="sm" asChild>
                                <Link href="/odcs"><ArrowLeft className="h-4 w-4" /></Link>
                            </Button>
                            <h1 className="text-2xl font-semibold">{odc.name}</h1>
                            <StatusBadge status={odc.status} />
                        </div>
                        <p className="text-sm text-muted-foreground font-mono ml-8">{odc.code}</p>
                    </div>
                    <Button asChild>
                        <Link href={`/odcs/${odc.id}/edit`}>
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-2xl font-bold">{odc.total_ports}</div>
                            <p className="text-xs text-muted-foreground">Total Port</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-2xl font-bold text-green-600">{odc.used_ports}</div>
                            <p className="text-xs text-muted-foreground">Port Terpakai</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-2xl font-bold text-blue-600">{odc.available_ports}</div>
                            <p className="text-xs text-muted-foreground">Port Tersedia</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-2xl font-bold">{odc.capacity_percentage}%</div>
                            <p className="text-xs text-muted-foreground">Utilisasi</p>
                            <CapacityBar usedPorts={odc.used_ports} totalPorts={odc.total_ports} showLabel={false} className="mt-2" />
                        </CardContent>
                    </Card>
                </div>
                ...
                <Tabs defaultValue="info">
                                    ...
                                    <div>
                                        ...
                                        <dl className="space-y-2 text-sm">
                                            <div className="flex justify-between">
                                                ...
                                                <dd className="font-mono font-medium">{odc.code}</dd>
                                            </div>
                                            <div className="flex justify-between">
                                            ...
                                            <div className="flex justify-between">
                                                <dt className="text-muted-foreground">OLT Parent</dt>
                                                <dd>
                                                    {odc.olt ? (
                                                        <Link href={`/olts/${odc.olt_id}`} className="hover:underline text-primary">
                                                            {odc.olt.name}
                                                        </Link>
                                                    ) : '—'}
                                                </dd>
                                    ...
                                    <div>
                                        ...
                                        <dl className="space-y-2 text-sm">
                                            ...
                                            <div className="flex justify-between">
                                                ...
                                                <dd>{odc.district?.name ?? '—'}</dd>
                                            </div>
                                            <div className="flex justify-between">
                                                <dt className="text-muted-foreground">Kelurahan</dt>
                                                <dd>{odc.village?.name ?? '—'}</dd>
                                            </div>
                                            <div className="flex justify-between">
                                                <dt className="text-muted-foreground">Koordinat</dt>
                                                <dd className="font-mono text-xs">
                                                    {odc.latitude != null ? `${odc.latitude}, ${odc.longitude}` : '—'}
                                                </dd>
                                            </div>
                                        </dl>
                                        {odc.location_description && (
                                ...
                                {odc.notes && (
                                    <>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm whitespace-pre-wrap">{odc.notes}</p>
                                        </div>
                                    </>
                                )}
                            </CardContent>
                    ...
                    {/* ODPs Tab */}
                    <TabsContent value="odps" className="mt-4">
                        <Card>
                            <CardHeader>
                                <CardTitle className="text-base">Daftar ODP</CardTitle>
                            </CardHeader>
                            <CardContent>
                                {!odc.odps || odc.odps.length === 0 ? (
                                    <p className="text-sm text-muted-foreground text-center py-8">Belum ada ODP terdaftar.</p>
                                ) : (
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                                <TableHead>Nama / Kode</TableHead>
                                                <TableHead>No. Tiang</TableHead>
                                        ...
                                        <TableBody>
                                            {odc.odps.map((odp) => (
                                                <TableRow key={odp.id}>
                                                    ...
                                                    <TableCell className="font-mono text-sm">{odp.pole_number ?? '—'}</TableCell>
                                                    <TableCell><StatusBadge status={odp.status} /></TableCell>
                                                    <TableCell>
                                                        <CapacityBar usedPorts={odp.used_ports} totalPorts={odp.total_ports} className="w-24" />
                                                    </TableCell>
                                                    <TableCell>
                                                        <Button variant="ghost" size="sm" asChild>
                                                            <Link href={`/odps/${odp.id}`}>Detail</Link>
                                                        </Button>
                                                    </TableCell>
                                                </TableRow>
                                            ))}
