...
import type { BreadcrumbItem, MaintenanceLog, Odp } from '@/types';
...
export default function OdpShow({ odp, maintenanceLogs }: Props) {
    useFlash();

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'ODP', href: '/odps' },
        { title: odp.name, href: `/odps/${odp.id}` },
    ];
    ...
    const parentPoint = odp.odc && odp.odc.latitude != null && odp.odc.longitude != null
        ? {
            latitude: odp.odc.latitude,
            longitude: odp.odc.longitude,
            label: `Parent ODC: ${odp.odc.code}`,
        }
            : null;

    const hasMapContext = (odp.latitude != null && odp.longitude != null) || parentPoint !== null;

    function submitMaintenance(e: React.FormEvent) {
        e.preventDefault();
        maintenanceForm.post(`/odps/${odp.id}/maintenance-logs`, {
            preserveScroll: true,
            ...
            <div className="p-6 space-y-6">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <Button variant="ghost" size="sm" asChild>
                                <Link href="/odps"><ArrowLeft className="h-4 w-4" /></Link>
                            </Button>
                            <h1 className="text-2xl font-semibold">{odp.name}</h1>
                            <StatusBadge status={odp.status} />
                        </div>
                        <p className="text-sm text-muted-foreground font-mono ml-8">{odp.code}</p>
                    </div>
                    <Button asChild>
                        <Link href={`/odps/${odp.id}/edit`}>
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-2xl font-bold">{odp.total_ports}</div>
                            <p className="text-xs text-muted-foreground">Total Port</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-2xl font-bold text-green-600">{odp.used_ports}</div>
                            <p className="text-xs text-muted-foreground">Port Terpakai</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-2xl font-bold text-blue-600">{odp.available_ports}</div>
                            <p className="text-xs text-muted-foreground">Port Tersedia</p>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-2xl font-bold">{odp.capacity_percentage}%</div>
                            <p className="text-xs text-muted-foreground">Utilisasi</p>
                            <CapacityBar usedPorts={odp.used_ports} totalPorts={odp.total_ports} showLabel={false} className="mt-2" />
                        </CardContent>
                    </Card>
                </div>
                ...
                <Tabs defaultValue="info">
                    <TabsList>
                        ...
                        <TabsTrigger value="onts">
                            ONT
                            {odp.onts && odp.onts.length > 0 && (
                                    ...
                                    <div>
                                        ...
                                        <dl className="space-y-2 text-sm">
                                            <div className="flex justify-between">
                                                ...
                                                <dd className="font-mono font-medium">{odp.code}</dd>
                                            </div>
                                            <div className="flex justify-between">
                                            ...
                                            <div className="flex justify-between">
                                                <dt className="text-muted-foreground">Parent ODC</dt>
                                                <dd>
                                                    {odp.odc ? (
                                                        <Link href={`/odcs/${odp.odc_id}`} className="hover:underline text-primary">
                                                            {odp.odc.code}
                                                        </Link>
                                                    ) : '—'}
                                                </dd>
                                            ...
                                            <div className="flex justify-between">
                                                ...
                                                <dd>{odp.installation_date ?? '—'}</dd>
                                            </div>
                                            <div className="flex justify-between">
                                                <dt className="text-muted-foreground">Maintenance Terakhir</dt>
                                                <dd>{odp.last_maintenance_date ?? '—'}</dd>
                                            </div>
                                        </dl>
                                    </div>
                                    <div>
                                        ...
                                        <dl className="space-y-2 text-sm">
                                            ...
                                            <div className="flex justify-between">
                                                ...
                                                <dd>{odp.district?.name ?? '—'}</dd>
                                            </div>
                                            <div className="flex justify-between">
                                                <dt className="text-muted-foreground">Kelurahan</dt>
                                                <dd>{odp.village?.name ?? '—'}</dd>
                                            </div>
                                            <div className="flex justify-between">
                                                <dt className="text-muted-foreground">No. Tiang</dt>
                                                <dd className="font-mono">{odp.pole_number ?? '—'}</dd>
                                            </div>
                                            <div className="flex justify-between">
                                ...
                                {odp.notes && (
                                    <>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm whitespace-pre-wrap">{odp.notes}</p>
                                        </div>
                                    </>
                                )}
                            </CardContent>
                    ...
                    <TabsContent value="onts" className="mt-4">
                        <Card>
                            ...
                            <CardContent>
                                {!odp.onts || odp.onts.length === 0 ? (
                                    <p className="text-sm text-muted-foreground text-center py-8">Belum ada ONT terdaftar pada ODP ini.</p>
                                ) : (
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                        ...
                                        <TableBody>
                                            {odp.onts.map((ont) => (
                                                <TableRow key={ont.id}>
                                                    ...
                                                    <TableCell className="text-sm text-muted-foreground">{ont.customer_name ?? '—'}</TableCell>
                                                    <TableCell><StatusBadge status={ont.status} /></TableCell>
                                                    ...
                                                    <TableCell>
                                                        <Button variant="ghost" size="sm" asChild>
                                                            <Link href={`/onts/${ont.id}`}>Detail</Link>
                                                        </Button>
                                                    </TableCell>
                                                </TableRow>
                                            ))}
                        ...
                        <Card>
                            <CardContent className="pt-6">
                                {hasMapContext ? (
                                    ...
                                    <p className="text-sm text-muted-foreground text-center py-8">Koordinat ODP maupun parent ODC belum diset.</p>
                                )}
                            </CardContent>
                        </Card>
                    </TabsContent>
